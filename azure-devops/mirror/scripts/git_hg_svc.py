...
class GitHgSVC(object):
    ...
    def remote_add_origin(self, origin_uri: str) -> None:
        """
        add a uri to the upstream remote
        """
        assert self.REMOTE_ORIGIN not in self.__repo.remotes
        self.__repo.create_remote(self.REMOTE_ORIGIN, origin_uri)
        assert self.__repo.remote(self.REMOTE_ORIGIN).exists()
    ...
    def push(self, upstream: str, local_branch: str, upstream_branch: str) -> None:
        ...
        self.__repo.git.push(upstream, "--tags", "-f")

    def remote_remove_origin(self) -> None:
        """
        remote the upstream remote from the local repo
        """
        assert self.__repo.remote(self.REMOTE_ORIGIN).exists()
        self.__repo.delete_remote(self.REMOTE_ORIGIN)
        assert self.REMOTE_ORIGIN not in self.__repo.remotes
